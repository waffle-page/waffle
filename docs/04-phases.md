...
| **Catalog product — separate track** | Proprietary entity graph + discovery | Rights-cleared provider-neutral master graph (see `07-catalog.md` and `16-catalog-product-and-entity-graph.md`); audited contribution protocol; text/vector/facet/geo search; home feed; privacy-thresholded community signals; source-by-source expansion only after legal and economic validation |

## P2 public-link publishing contract

Public-link publishing is not collaborative sharing and is not Catalog product
